...
backup_to_gdrive() {
    ...
    if [ "$GDRIVE_CONFIGURED" = false ]; then
        ...
        echo ""
        log_warning "⚠️  Google Drive requires OAuth credentials - this is complex!"
        log_info "Recommended: Use Backblaze B2 instead (much simpler!)"
        echo ""
        
        if ! prompt_yes_no "Continue with Google Drive anyway?"; then
            log_info "Smart choice! Let's use Backblaze B2 instead."
            backup_to_backblaze
            return
        fi
        ...
        echo ""
        log_info "╔════════════════════════════════════════════════════════════════════╗"
        log_info "║  STEP 1: Create Google OAuth Credentials (on your local PC)       ║"
        log_info "╚════════════════════════════════════════════════════════════════════╝"
        echo ""
        ...
        echo ""
        log_info "Have your Client ID and Client Secret ready!"
        echo ""
        read -p "Press Enter when you have your credentials..."
        
        echo ""
        log_info "╔════════════════════════════════════════════════════════════════════╗"
        log_info "║  STEP 2: Configure rclone (on this server)                        ║"
        log_info "╚════════════════════════════════════════════════════════════════════╝"
        echo ""
        echo "  1. When rclone config opens, choose: n (New remote)"
        echo "  2. Name: gdrive"
        echo "  3. Storage: 18 (or type 'drive' for Google Drive)"
        echo "  4. Client ID: [PASTE YOUR CLIENT ID]"
        echo "  5. Client Secret: [PASTE YOUR CLIENT SECRET]"
